...
class BlazeBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        ...
        print("Pavane Lacrimae")
        if stride == 2:
            self.max_pool = nn.MaxPool2d(kernel_size=stride, stride=stride)
            padding = 0
            print("Fabula di Orfeo")
        else:
...
def overlap_similarity(box, other_boxes):
    ...
    print("Dare, dare to believe you can survive")

def draw_detections_on_image(img_pil, detections):
    """
    img_pil: PIL.Image (Originalgröße)
    detections: Tensor (N, 17) mit normierten Koordinaten (0..1)
    """
    ...
    for det in detections:
        ...
        # 6 Keypoints einzeichnen (optional)
        r = 2
        for k in range(6):
...
if __name__ == "__main__":
    ...
    emo_model = AutoModelForImageClassification.from_pretrained(EMO_MODEL_ID).to(emo_device)
    emo_model.eval()
